  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (plowState.current.isPlowing) {
        ...
        // --- TRAILER (Aanhanger) ---
        // Position: Behind the truck (negative x relative to truck origin)
        const trailerX = -110;
        const trailerY = -35;
        const trailerW = 100;
        const trailerH = 35;
        ...
        // Trailer Body (Darker Red/Brown)
        ctx.fillStyle = '#b91c1c';
        ctx.fillRect(trailerX, trailerY, trailerW, trailerH);

        // Trailer Rim/Top
        ctx.fillStyle = '#7f1d1d';
        ctx.fillRect(trailerX, trailerY, trailerW, 5);
        ...
        ctx.beginPath();
        ctx.moveTo(trailerX + 10, trailerY);
        ctx.lineTo(trailerX + trailerW / 2, trailerY - 10);
        ctx.lineTo(trailerX + trailerW - 10, trailerY);
        ctx.fill();

        // Shadow
        ctx.fillStyle = 'rgba(0,0,0,0.3)';
        ctx.beginPath(); ctx.ellipse(60, 5, 70, 10, 0, 0, Math.PI * 2); ctx.fill(); // Truck shadow
        ctx.beginPath(); ctx.ellipse(trailerX + trailerW / 2, 5, trailerW / 2 + 10, 8, 0, 0, Math.PI * 2); ctx.fill(); // Trailer shadow
        ...
        // Text on Truck (Keep PARKSTAD or remove? User didn't say remove, but said put text on trailer. I'll keep PARKSTAD on truck for continuity or maybe change to logo)
        ctx.fillStyle = 'white';
        ctx.font = '900 10px Arial';
        ctx.textAlign = 'left';
        ctx.fillText('KERST', 5, -5); // Changed to KERST to match theme
